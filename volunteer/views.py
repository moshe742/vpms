...
# from volunteer.forms import Add_project, Add_volunteer
...
def home(request):
    ...
    return render(request, 'volunteer/home.html', {'eknights': eknight, 'message': 'message', 'links': links})


def arrived(request):
    arrive = Arrival.objects.filter(user_arrived=datetime.date.today())
    # volunteer = Volunteer.objects.filter(id=arrive.user_id)
    return render(request, 'volunteer/arrived.html', {'arrived': arrive })
...
def welcome(request):
    ...
    eknight_id = Project.objects.get(id=eknight_id)
...
def add_project(request):
    ...
    return render(request, 'volunteer/add_project.html', {'form': form, 'project_list': project_list})
...
def add_volunteer(request, eknight_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            arrival.save()
            return HttpResponseRedirect('/')
    return render(request, 'volunteer/add_volunteer.html', {'form': form})


def enter_project(request):
    project_list = EKnight.objects.all().order_by('name')
    return render(request, 'volunteer/enter_project.html', {'project_list': project_list})


def enter_name(request, project_id):
    name = Volunteer.objects.all().order_by('name')
    project = get_object_or_404(EKnight, pk=project_id)
    return render(request, 'volunteer/enter_name.html', {'name_list': name, 'project': project})
...
def feedback(request):
    ...
    return render(request, 'volunteer/feedback.html', {form: form})
...
@permission_required("auth.admin", login_url="/admin/login")
def vol_dates(request):
    ...
    return render(request, 'volunteer/vol_date.html', {'vol': volunteer, 'date': dates})
...
def volunteer_data(request, v_id):
    ...
    if request.user.is_authenticated():
        ...
        logged_in = False
    return render(request, "volunteer/vol_data.html", {"vol": vol, "date": vol_date, "logged_in": logged_in})


@permission_required("auth.admin", login_url="/admin/login")
def date_data(request, user_arrived):
    vol_dates = Arrival.objects.filter(user_arrived=user_arrived)
    return render(request, "volunteer/date_data.html", {"date": vol_dates})
